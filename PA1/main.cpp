...
int main(int argc, char **argv) {
  ...
  MPI_Init(&argc, &argv); 
  // printf("MPI_Init %d ... %s ... %s ... %s \n", argc, argv[0], argv[1], argv[2]);

  int nprocs, rank;  //number of processes, rank of process
  CHKERR(MPI_Comm_size(MPI_COMM_WORLD, &nprocs)); // number of processes
  CHKERR(MPI_Comm_rank(MPI_COMM_WORLD, &rank)); // rank of process

  // printf("nprocs = %d, rank %d\n", nprocs, rank);

  // check if srun command is correct
  if (argc != 3) {
    if (!rank)
  ...
  const char *input_name = argv[2]; // name of file

  // number of elements less than number of processes
  // not every process will become useful
  if (n < nprocs) 
  ...
  // make the different workers (which controls each process)
